...
argument_test_fixture::argument_ptr_type prepare_argument(std::string_view name) {
    return std::make_unique<positional_argument<test_value_type>>(name);
}

argument_test_fixture::argument_ptr_type
prepare_argument(std::string_view name, std::string_view long_name) {
    return std::make_unique<positional_argument<test_value_type>>(name, long_name);
}
...
TEST_CASE_FIXTURE(argument_test_fixture, "is_optional() should return false") {
...
TEST_CASE_FIXTURE(argument_test_fixture, "has_value() should return true is value is set") {
...
TEST_CASE_FIXTURE(
    argument_test_fixture,
    "is_required() should return true regardles of the "
    "requried(bool) function usage"
) {
    auto sut = prepare_argument(long_name);
...
TEST_CASE_FIXTURE(argument_test_fixture, "defaul_value() should return null any instance by default") {
    const auto sut = prepare_argument(long_name);
...
TEST_CASE_FIXTURE(
    argument_test_fixture,
    "defaul_value() should return null any instance regardles of "
    "the default_value(const std::any&) function usage"
) {
    auto sut = prepare_argument(long_name);
    ...
    REQUIRE_FALSE(stored_default_value.has_value());
}
